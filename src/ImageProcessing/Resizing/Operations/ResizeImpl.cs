...
internal static class ResizeImpl
{
    ...
    private static Rgb GetPixelClamped_Rgb(ReadOnlySpan<Rgb> src, int originalWidth, int originalHeight, int x, int y)
    {
        int cx = Math.Clamp(x, 0, originalWidth - 1);
        int cy = Math.Clamp(y, 0, originalHeight - 1);
        var outPixel = src[cy * originalWidth + cx];
    ...
    public static void ResizeBicubicByte(ReadOnlySpan<byte> byteSrc, Span<byte> byteDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
    ...
    public static void ResizeBicubicRgb(ReadOnlySpan<Rgb> rgbSrc, Span<Rgb> rgbDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bicubic_interpolation
            ...
            // 1st row
            var p00 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint - 1, yint - 1);
            var p01 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 0, yint - 1);
            var p02 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 1, yint - 1);
            var p03 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 2, yint - 1);

            // 2nd row
            var p10 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint - 1, yint + 0);
            var p11 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 0, yint + 0);
            var p12 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 1, yint + 0);
            var p13 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 2, yint + 0);

            // 3rd row
            var p20 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint - 1, yint + 1);
            var p21 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 0, yint + 1);
            var p22 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 1, yint + 1);
            var p23 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 2, yint + 1);

            // 4th row
            var p30 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint - 1, yint + 2);
            var p31 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 0, yint + 2);
            var p32 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 1, yint + 2);
            var p33 = GetPixelClamped_Rgb(rgbSrc, originalWidth, originalHeight, xint + 2, yint + 2);
    ...
    public static void ResizeBicubicRgb24(ReadOnlySpan<Rgb24> rgb24Src, Span<Rgb24> rgb24Dest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bicubic_interpolation
            ...
            // 1st row
            var p00 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint - 1, yint - 1);
            var p01 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 0, yint - 1);
            var p02 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 1, yint - 1);
            var p03 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 2, yint - 1);

            // 2nd row
            var p10 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint - 1, yint + 0);
            var p11 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 0, yint + 0);
            var p12 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 1, yint + 0);
            var p13 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 2, yint + 0);

            // 3rd row
            var p20 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint - 1, yint + 1);
            var p21 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 0, yint + 1);
            var p22 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 1, yint + 1);
            var p23 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 2, yint + 1);

            // 4th row
            var p30 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint - 1, yint + 2);
            var p31 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 0, yint + 2);
            var p32 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 1, yint + 2);
            var p33 = GetPixelClamped_Rgb24(rgb24Src, originalWidth, originalHeight, xint + 2, yint + 2);
    ...
    public static void ResizeBicubicRgb48(ReadOnlySpan<Rgb48> rgb48Src, Span<Rgb48> rgb48Dest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bicubic_interpolation
            ...
            // 1st row
            var p00 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint - 1, yint - 1);
            var p01 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 0, yint - 1);
            var p02 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 1, yint - 1);
            var p03 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 2, yint - 1);

            // 2nd row
            var p10 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint - 1, yint + 0);
            var p11 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 0, yint + 0);
            var p12 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 1, yint + 0);
            var p13 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 2, yint + 0);

            // 3rd row
            var p20 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint - 1, yint + 1);
            var p21 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 0, yint + 1);
            var p22 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 1, yint + 1);
            var p23 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 2, yint + 1);

            // 4th row
            var p30 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint - 1, yint + 2);
            var p31 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 0, yint + 2);
            var p32 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 1, yint + 2);
            var p33 = GetPixelClamped_Rgb48(rgb48Src, originalWidth, originalHeight, xint + 2, yint + 2);
    ...
    public static void ResizeBicubicSingle(ReadOnlySpan<float> byteSrc, Span<float> byteDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bicubic_interpolation
    ...
    public static void ResizeBicubicUInt16(ReadOnlySpan<ushort> byteSrc, Span<ushort> byteDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bicubic_interpolation
    ...
    public static void ResizeBilinearByte(ReadOnlySpan<byte> byteSrc, Span<byte> byteDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
    ...
    public static void ResizeBilinearRgb(ReadOnlySpan<Rgb> byteSrc, Span<Rgb> byteDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bilinear_interpolation
    ...
    public static void ResizeBilinearRgb24(ReadOnlySpan<Rgb24> byteSrc, Span<Rgb24> byteDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bilinear_interpolation
    ...
    public static void ResizeBilinearRgb48(ReadOnlySpan<Rgb48> byteSrc, Span<Rgb48> byteDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bilinear_interpolation
    ...
    public static void ResizeBilinearSingle(ReadOnlySpan<float> byteSrc, Span<float> byteDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bilinear_interpolation
    ...
    public static void ResizeBilinearUInt16(ReadOnlySpan<ushort> byteSrc, Span<ushort> byteDest, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            // see https://en.wikipedia.org/wiki/Bilinear_interpolation
    ...
    public static void ResizeNearestNeighborByte(ReadOnlySpan<byte> source, Span<byte> destination, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            int destX = xDest;
    ...
    public static void ResizeNearestNeighborRgb(ReadOnlySpan<Rgb> source, Span<Rgb> destination, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            int destX = xDest;
    ...
    public static void ResizeNearestNeighborRgb24(ReadOnlySpan<Rgb24> source, Span<Rgb24> destination, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            int destX = xDest;
    ...
    public static void ResizeNearestNeighborRgb48(ReadOnlySpan<Rgb48> source, Span<Rgb48> destination, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            int destX = xDest;
    ...
    public static void ResizeNearestNeighborSingle(ReadOnlySpan<float> source, Span<float> destination, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            int destX = xDest;
    ...
    public static void ResizeNearestNeighborUInt16(ReadOnlySpan<ushort> source, Span<ushort> destination, int originalWidth, int originalHeight, int destinationWidth, int destinationHeight, int yDest)
    {
        for (int xDest = 0; xDest < destinationWidth; xDest++)
        {
            int destX = xDest;
